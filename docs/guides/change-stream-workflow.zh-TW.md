...
flowchart TD
    ...
    Direct -->|dev work park --next| WarmDirect["WARM: 保留 checkout"]
    WarmDirect -->|dev work resume| Direct
    Managed -->|dev work park --next| WarmManaged["WARM: 保留 checkout"]
    WarmManaged -->|dev work resume| Managed
    Managed -->|dev work park --cold --push| Cold["COLD: 已 push 且可重建"]
    WarmManaged -->|dev work park --cold --push| Cold
    Cold -->|dev work resume --fetch| Managed

    Direct -->|dev work done| Done["DONE / MERGED"]
    WarmDirect -->|dev work done| Done
    Managed -->|dev work done --ff| Done
    WarmManaged -->|dev work done --ff| Done

    Managed -->|dev work done --pr| Review["push / review handoff: state 不變"]
    WarmManaged -->|dev work done --pr| Review
    Review -->|feedback：若為 WARM 先 dev work resume| Managed
    Review -->|dev work done --merged --base-ref REF| Done

    Done -->|dev work retire| Retired["RETIRED: guarded cleanup 完成"]
    Done -->|dev work sweep| Report["先回報 cleanup candidates"]
    Report -->|dev work sweep --apply| Retired
    Retired --> Next["下一條 change stream"]
    Next --> Start
```

Handoff path 刻意保持 state：`dev work done --pr` 會讓 task 保持 HOT/WARM；沒有支援的 forge CLI 時也可能只 push。`dev` 不會自動把 provider 的 merged 狀態當成 local DONE。Merge 完成後，以 `dev work done --merged --base-ref <ref>` 或 `dev repo flow` 的 Verify Merged action 驗證 named ancestry；記錄 DONE 之後，再以 Retire 做 cleanup。

## 1. 選擇 checkout mode

| Mode | 命令 | 適用情境 |
|---|---|---|
| direct | `dev work start <repo> --task <name> --direct` | canonical checkout 中的一個小型可逆變更 |
| branch-only | `dev work start <repo> --task <name> --branch-only --base <ref>` | 需要 history isolation，但不需要第二個 directory |
| worktree | `dev work start <repo> --task <name> --base <ref>` | 獨立寫入、平行工作或會比 session 存在更久的變更流 (change stream) |
...
Start 會解析 repository、驗證 branch/base、建立或切換 checkout、需要時佈建 worktree、開啟 runtime，再保存 task。Branch 已被追蹤時應使用 `dev work resume`，不要建立重複 task。

需要把 intent、目前 Git/runtime/artifact evidence 與可執行 action 並排檢查時，可從 TTY 開啟獨立 preview：

```bash
dev repo flow              # 目前 checkout 的 canonical repository 與 exact surface
dev repo flow api          # 明確指定 repository
```

`dev repo flow` 顯示 Git 註冊的所有 worktrees 與沒有 checkout 的 task-only rows。Enter 只建立 fresh guarded plan；plan 為 READY 且經第二次 approval 後才 Apply。小寫 `r` 只更新 local facts，大寫 `R` 才選擇 fetch refs、query exact PR/MR 或兩者。完整 row/action 與 safety boundary 見 [Repository Flow 預覽](repository-flow.zh-TW.md)。
...
- `dev work park --note` 保存這一個 task 的 free-form context；
- `dev repo mark --note` 取代 catalog 的單一 repository summary；
- `dev repo note` 保存多筆 durable repository observations，不屬於 task lifecycle state。
...
這要求 clean tree，先把 task branch rebase 到 base，再於 canonical checkout 執行 fast-forward-only merge，最後標示 task DONE/MERGED。它不會關閉呼叫端 runtime、移除 worktree 或刪除 branch；請在外部 workspace 另行執行 `dev work retire`。
...
這會 push branch，並在對應 CLI 可用時建立 pull/merge request。它**不會**標示 task DONE，也不會清理 checkout。`dev work sweep` 不會推斷 remote request 已 merge；`dev repo flow` 的明確 `R` query 也只保留目前 run 的 provider evidence，不會自動 transition。Merge 後請驗證 exact named ref：

```bash
git fetch origin
dev work done --merged --base-ref origin/main
...
## 互動式 `dev work done` finish wizard

在 interactive terminal 上，省略 `--ff` 與 `--pr` 執行 `dev work done` 不會直接失敗，而是開啟 finish wizard。它不會猜你要哪種 integration mode，而是先檢視 checkout 的實際狀態，只詢問它無法推斷的部分。
...
3. **Integration**，僅在未傳入 `--ff` 或 `--pr`、且 branch 尚未完全被 base 包含時出現：`f` 把 branch rebase 到 base 再 fast-forward（等同 `--ff`），`p` push 並開啟 pull/merge request（等同 `--pr`），`q` 取消。Branch 已被 base 包含時，wizard 會跳過此步驟，直接記錄 DONE/MERGED；runtime/worktree cleanup 仍留給之後的 `dev work retire`。
...
Non-interactive 情境 —— 沒有 TTY，或在 script 中 —— wizard 完全不會詢問。未指定 integration flag 時 `dev work done` 只會回報同樣的 preflight 後結束；請明確傳入 `--ff` 或 `--pr`。Dirty checkout 在沒有 TTY 時預設失敗（`--dirty auto` 等同 `--dirty fail`），因此 script 應選擇明確的 policy：

```bash
dev work done --ff --dirty commit -m "chore: finalize before merge"
dev work done --pr --dirty discard --yes   # destructive；此處 --yes 為必要
```
...
```bash
dev work retire <task>                  # 預設保留 branch
dev work retire <task> --delete-branch  # 另行確認 freshly contained branch deletion
```
...
```bash
dev work sweep                 # 只回報
dev work sweep --apply         # 每個建議個別確認
dev work sweep --apply --yes   # 已檢查 report 後的 automation
```
...
- Rebase conflict：解決後執行 `git rebase --continue`，再重跑 `dev work done --ff`；要回到 rebase 前狀態則用 `git rebase --abort`。
- Worktree directory 消失：先用 `dev work sweep` 或 `dev git worktree rm` 清理 stale Git administration，再 resume。
