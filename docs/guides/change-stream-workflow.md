...
flowchart TD
    ...
    Start["dev work start"] --> Mode{"checkout mode"}
    Mode -->|direct| Direct["HOT: direct work / commit / test"]
    Mode -->|branch-only or worktree| Managed["HOT: managed work / commit / test"]

    Direct -->|dev work park --next| WarmDirect["WARM: checkout kept"]
    WarmDirect -->|dev work resume| Direct
    Managed -->|dev work park --next| WarmManaged["WARM: checkout kept"]
    WarmManaged -->|dev work resume| Managed
    Managed -->|dev work park --cold --push| Cold["COLD: pushed and reconstructible"]
    WarmManaged -->|dev work park --cold --push| Cold
    Cold -->|dev work resume --fetch| Managed

    Direct -->|dev work done| Done["DONE: integration confirmed"]
    WarmDirect -->|dev work done| Done
    Managed -->|dev work done --ff| Done
    WarmManaged -->|dev work done --ff| Done

    Managed -->|dev work done --pr| Review["push / review handoff: task stays active"]
    WarmManaged -->|dev work done --pr| Review
    Review -->|feedback: dev work resume if WARM| Managed
    Review -->|merged externally| Reconcile["dev work done --merged: verify exact ancestry"]
    Reconcile --> Done

    Done -->|dev work retire from outside target| Reaped["RETIRED: clean resources and reap task"]
    Done -.->|dev work sweep| Report["report cleanup candidates"]
    Report -.->|approved apply| Reaped
    Reaped --> Next["next change stream"]
    Next --> Start
```

The handoff path deliberately stops at manual reconciliation: `dev work done --pr` leaves the task active and may only push when no supported forge CLI is available. `dev repo flow` can explicitly query run-local review existence/state/draft/URL evidence, but neither it nor `sweep` infers DONE from a forge response. Use `dev work done --merged` with exact local ancestry evidence.

## 1. Select the checkout mode

| Mode | Command | Use when |
|---|---|---|
| direct | `dev work start <repo> --task <name> --direct` | one small reversible change in the canonical checkout |
| branch-only | `dev work start <repo> --task <name> --branch-only --base <ref>` | history isolation is useful but a second directory is not |
| worktree | `dev work start <repo> --task <name> --base <ref>` | independent mutation, parallel work, or a stream that may outlive the session |
...
```bash
dev work start api --task "token refresh" --base main --next "reproduce expiry race"
dev work list
dev status
dev repo flow api   # preview all repository surfaces and exact guarded actions
```

Starting work resolves the repository, validates branch/base, creates or switches the checkout, provisions a worktree when needed, opens the selected runtime, and saves the task. If the branch is already tracked, use `dev work resume` instead of creating a duplicate.

## 3. Park intentionally

```bash
dev work park --next "add the failing expiry test"
```
...
- `dev work park --note` preserves free-form context for this one task;
- `dev repo mark --note` replaces the catalog's single repository summary;
- `dev repo note` stores multiple durable repository observations outside task lifecycle state.

If the tree is dirty:

```bash
dev work park --wip --next "finish the expiry test"
```
...
This requires a clean tree, rebases the task branch onto the base when needed, and runs a fast-forward-only merge in the canonical checkout. It records DONE last while retaining the runtime, worktree, and branch. Run `dev work retire` separately from outside the target.
...
This pushes the branch and opens a GitHub pull request or GitLab merge request when the corresponding CLI is available. It **does not** mark the task DONE or clean up the checkout. Current `dev work sweep` also does not infer remote request merges, so confirm integration before finishing the local lifecycle.

## The interactive `dev work done` finish wizard

Running `dev work done` with neither `--ff` nor `--pr` on an interactive terminal opens a finish wizard instead of failing. It never guesses which integration mode you want; it walks the actual state of the checkout and asks only what it cannot infer.
...
3. **Integration**, only if neither `--ff` nor `--pr` was passed and the branch is not already fully contained in the base: `f` rebases the branch onto the base and fast-forwards it (same as `--ff`), `p` pushes and opens a pull/merge request (same as `--pr`), `q` cancels. When the branch is already contained in the base, the wizard skips this step and can record DONE; cleanup still belongs to `dev work retire`.
...
Non-interactively — no TTY, or a script — the wizard never prompts. `dev work done` with no integration flag just reports the same preflight and exits; pass `--ff` or `--pr` explicitly. A dirty checkout defaults to failing outside a TTY (`--dirty auto` behaves like `--dirty fail`), so scripts choose an explicit policy:

```bash
dev work done --ff --dirty commit -m "chore: finalize before merge"
dev work done --pr --dirty discard --yes   # destructive; --yes is mandatory here
```
...
Use `dev repo flow [repo]` when the question is not only “what state is this task?”
...
dev work sweep --apply         # confirm each suggested action
dev work sweep --apply --yes   # automation after reviewing the report
```
...
- Rebase conflict: resolve it, run `git rebase --continue`, then retry `dev work done --ff`; use `git rebase --abort` to return to the pre-rebase state.
- Missing worktree directory: run `dev work sweep` or `dev git worktree rm` to prune stale Git administration, then resume.
