...
|---|---|---|---|
| feature、fix、experiment、跨機器 handoff | `dev` | 設定的 `paths.worktree_path` | `dev work done` 只記錄 MERGED；直到後續 `dev work retire`、明確 `dev git worktree rm` 或核准的 sweep 才移除 |
...
| 外部建立的 linked worktree | adoption 前由外部管理 | tool-specific | `dev`/`dev repo flow` 可看見；只有明確 Adopt 後才有 managed lifecycle |
...
dev git worktree plan --write          # 建立 repository-owned .dev-cli/config.toml
dev git worktree create feat/auth --base main
dev git worktree list
```

`dev git worktree plan` 會讀取 lockfile、tool availability、include/link 規則與 Git ignore state，但不修改 checkout。Plan 列出每個可執行或 skipped step，以及任何 safety downgrade。
...
command 必須先用 `dev self config trust . --yes` 核准其 exact executable-config hash
