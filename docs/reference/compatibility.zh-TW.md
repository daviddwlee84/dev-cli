...
- Interactive FF recovery 只會關閉 freshly revalidated、agent 為 `idle`/`done` 的 non-caller Herdr pane；working/blocked/waiting/unknown 仍會阻擋。Dirty canonical integration target 只能透過畫面顯示的 typed `DROP` taskflow plan discard；沒有 non-interactive flag，也不會自動 commit 不相干的 bytes。
