...
- `repo clone --fork` 與 `repo fork [repo-or-path]` 需要已登入的 `gh`，僅支援個人 GitHub fork。它們會驗證 fork/source identity，將 `origin` 設為 fork、`upstream` 設為 source，保留 branch pull target，並把預設與原本指向 source 的 push 設定導向 fork。Remote 衝突或指向無關目標的 explicit push 設定會阻擋套用。Fork acquisition 會 clone 目前 source，不受個人 fork 是否過時影響。Dry-run 只讀；非互動套用需要 `--yes`。不隱含執行 setup、push 或 PR，失敗時保留已完成 effects；普通 clone 行為不變。
...
Skills management wizard 使用直接安裝的 `skills`（支援相容的 1.x，最低 1.5.23），
缺少時不執行 npx。一般 inventory 與 Go 更新檢查不依賴 provider。
Experimental install 是上游依來源／ref 重裝，可能更新 lock，不保證舊 hash。
...
Help 與 Ctrl+O 共用浮窗，最大 104 欄 × 32 列；少於 80 欄或 22 列時
使用全畫面。可點擊控制、拖曳捲軸、Help 放大與文章橫向捲動支援觸控。
關閉或捲動浮窗不會啟動下方 dashboard。既有 actions、approval 條件、
顏色語意、CLI flags 與 JSON 維持不變。詳見
...
`--skill=true`、`skill print` 與 `help` 印出內容時，跳過 application
config 載入、release 提示／檢查及 Windows 升級遺留 binary 清理。
...
與完整 Windows suite 都是必要 gate；測試通過不擴大明確未支援的 mutation 範圍。
...
請見 [dashboard 指南](../guides/dashboard-actions.zh-TW.md)。
