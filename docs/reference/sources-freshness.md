...
| SSH key destinations, named agents/v2, FIDO and explicit vault creation | [SSH key providers](../guides/ssh-hosts.md) | `internal/sshhost`, `internal/sshvault`, CLI/TUI regressions and isolated native-fixture smokes | v0.2.38; real vault/hardware operations not exercised |
...
| release archives, native source upgrade fallback, package-manager ownership, and Homebrew tap publication | [Compatibility](compatibility.md) | `internal/cli/upgrade.go`, `internal/selfupdate`, `.github/workflows/release.yml`, `scripts/update-homebrew-formula.sh`, packaging helper test | implemented |
