...
Public-key bootstrap requires an explicit key or explicit Ed25519 generation and installs public material only. Config-only and existing-auth setup do not install a key. Bootstrap supports bounded ProxyJump forms plus fixed POSIX and Windows OpenSSH installers; `ProxyCommand`, custom `AuthorizedKeysFile`, forced-shell policy, and ACL-incapable filesystems require manual remediation. Once a remote installer starts, cancellation/failure is `unknown` because the key may have been appended. Dev retains local config/generated keys and never attempts key revocation, private-key deletion/copying, `known_hosts` cleanup, or automatic credential rollback. Password saving is a separate explicitly selected credential flow. Ordinary setup preserves foreign directives; source-aware onboarding and the SSH dashboard use the reviewed workflows described below.
...
- `repo clone --fork` and `repo fork [repo-or-path]` require authenticated `gh` and support personal GitHub forks only. They verify fork/source identity, configure `origin` as the fork and `upstream` as the source, preserve branch pull targets, and direct default/source-directed pushes to the fork. Conflicting remotes and unrelated explicit push targets block application. Fork acquisition clones the current source even when the personal fork is old. Dry-run is read-only; non-interactive application requires `--yes`. No implicit setup, push, or PR is performed, and failures retain completed effects. Ordinary clone behavior is unchanged.
...
- [Central Homebrew formula synchronization](https://github.com/daviddwlee84/homebrew-tap#one-writer-for-formulas)
