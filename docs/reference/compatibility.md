...
- Explicit and non-interactive `dev done` records MERGED only and keeps runtime/worktree/branch. Bare interactive completion may continue into a separate cleanup wizard: it previews runtime agents and can hand caller-owned Herdr cleanup to a newly created external coordinator, which still uses ordinary `dev retire` guards. Active agents and mixed-purpose workspaces remain blockers. `dev done --delete-branch` remains an error pointing at `dev retire --delete-branch`, and `--keep-worktree` warns as a no-op.
- Interactive FF recovery closes only freshly revalidated non-caller Herdr panes with `idle`/`done` agents. Working/blocked/waiting/unknown agents remain blockers. Dirty canonical integration targets may be discarded only through the displayed typed `DROP` taskflow plan; there is no non-interactive flag or automatic commit for those unrelated bytes.
