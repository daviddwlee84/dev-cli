...
// Target is the small project boundary needed by this package. Repository may
// be a catalog ID or display name; Path must identify the checkout root.
type Target struct {
	Repository string `json:"repo,omitempty"`
...
func SortDeclarations(rows []Declaration) {
	sort.SliceStable(rows, func(i, j int) bool {
		a, b := rows[i], rows[j]
		ak := []string{string(a.Agent), scopeSortKey(a.Scope), strings.ToLower(a.Repository), a.ProjectPath, a.ConfigPath, string(a.Source), strings.ToLower(a.Plugin), strings.ToLower(a.Name)}
		bk := []string{string(b.Agent), scopeSortKey(b.Scope), strings.ToLower(b.Repository), b.ProjectPath, b.ConfigPath, string(b.Source), strings.ToLower(b.Plugin), strings.ToLower(b.Name)}
		for n := range ak {
			if ak[n] != bk[n] {
				return ak[n] < bk[n]
			}
