...
func parseClaudeUser(data []byte, spec sourceSpec, targets []Target) ([]Declaration, []DiagnosticCode) {
	var document claudeDocument
	if code := decodeJSON(data, &document); code != "" {
		return nil, []DiagnosticCode{code}
	}
	...
	for _, projectPath := range projectKeys {
		...
		localSpec.repository = target.Repository
		localSpec.projectPath = target.Path
...
func convertClaudeServers(raw map[string]json.RawMessage, spec sourceSpec, state *claudeProjectState) ([]Declaration, []DiagnosticCode) {
	...
	for _, rawName := range sortedRawNames(raw) {
		...
		if state != nil {
			row.Trusted = copyBool(state.trusted)
...
func matchClaudeTarget(projectPath string, targets []Target) (Target, bool) {
	...
	for _, target := range targets {
		...
		targetReal := target.Path
		if resolved, err := filepath.EvalSymlinks(target.Path); err == nil {
			targetReal = resolved
		}
		if candidateReal == targetReal {
			return target, true
		}
	}
	return Target{}, false
