...
func normalizedName(value string) (string, bool) {
	...
	if value == "" || len(value) > maximumNameBytes || !utf8.ValidString(value) || stringContainsControl(value) || looksSecretString(value) {
		return "", false
	}
	return strings.ToLower(strings.Join(strings.Fields(value), " ")), true
}
...
func newDeclaration(spec sourceSpec, rawName string) (Declaration, bool) {
	...
	return Declaration{
		...
		Repository: spec.repository, ProjectPath: spec.projectPath,
		ConfigPath: spec.path, Source: spec.source, Transport: TransportUnknown,
	}, true
}
...
func inspectReferences(row *Declaration, value string) {
	...
	patterns := []*regexp.Regexp{dollarBracePattern, dollarPattern, percentPattern, openCodeEnvPattern}
...
func validEnvironmentName(value string) bool {
	return len(value) <= maximumReferenceBytes && environmentNamePattern.MatchString(value)
}

func validHeaderName(value string) bool {
	return len(value) <= maximumReferenceBytes && headerNamePattern.MatchString(value)
}
...
func setCommand(row *Declaration, command string) {
	...
	if basename == "." || basename == "/" || basename == "" || len(basename) > maximumNameBytes || looksSecretString(basename) {
		addRedaction(row, RedactionDynamicCommand)
		return
	}
	if strings.Contains(basename, "${") || strings.Contains(basename, "{env:") || dollarPattern.MatchString(basename) || percentPattern.MatchString(basename) {
		row.Command = "[dynamic]"
		addRedaction(row, RedactionDynamicCommand)
		return
	}
