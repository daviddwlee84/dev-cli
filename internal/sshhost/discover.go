...
// Discover is a convenience wrapper for callers that do not need a long-lived
// service or Runner.
func Discover(ctx context.Context, paths Paths, options DiscoverOptions) (Inventory, error) {
	service, err := NewService(paths, nil, ServiceOptions{Discovery: options})
	if err != nil {
		return Inventory{}, err
	}
		...
		case strings.EqualFold(directive, "include"):
			...
			if current.set {
				...
				if current.guard.Dynamic {
					s.addDiagnostic(Diagnostic{
						Code: "dynamic_match_include", Message: "Include is guarded by Match criteria that static discovery does not execute",
						Path: cleanPath, Source: &location, Incomplete: true, BlocksMutation: true,
					})
				}
			}
			...
			for _, argument := range arguments {
				next, _, scanErr := s.scanInclude(ctx, argument, location, depth, provenance, includeConstraints, current)
				if scanErr != nil {
...
func (s *scanner) scanInclude(ctx context.Context, argument string, source Location, depth int, provenance []IncludeFrame, constraints []Guard, initial guardState) (guardState, bool, error) {
	...
	for _, match := range matches {
		...
		if scanErr != nil {
			return flow, changed, scanErr
		}
		if !guardStatesEqual(flow, next) {
			changed = true
		}
		flow = next
	}
...
func evaluateMatchGuard(alias string, arguments []string) (bool, bool) {
	...
	if len(arguments) == 2 && (strings.EqualFold(arguments[0], "host") || strings.EqualFold(arguments[0], "originalhost")) {
...
func guardStatesEqual(left, right guardState) bool {
	if left.set != right.set {
		return false
	}
	return !left.set || guardsEqual(left.guard, right.guard)
}
