...
func (s *Service) evaluateSSHConfig(ctx context.Context, alias string, args []string, display, errorContext string, stderrOnExit bool) (EffectiveConfig, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	result, err := s.runner.Run(ctx, RunRequest{
		Name: "ssh", Args: args, Env: []string{"LC_ALL=C"}, Display: display,
	})
	if err != nil {
		return EffectiveConfig{}, fmt.Errorf("%s: %w", errorContext, err)
	}
	if result.ExitCode != 0 {
		...
		return EffectiveConfig{}, fmt.Errorf("%s: %s", errorContext, message)
	}
...
func firstEffectiveValue(effective EffectiveConfig, key string) string {
	if values := effective.Values[key]; len(values) > 0 {
		return values[0]
	}
	return ""
}
...
func ParseEffective(alias string, output []byte) (EffectiveConfig, error) {
	...
	config.HostName = firstEffectiveValue(config, "hostname")
	config.User = firstEffectiveValue(config, "user")
	config.ProxyJump = firstEffectiveValue(config, "proxyjump")
	config.IdentityFiles = append([]string(nil), config.Values["identityfile"]...)
	if value := firstEffectiveValue(config, "port"); value != "" {
	...
	if value := firstEffectiveValue(config, "identitiesonly"); value != "" {
		var enabled bool
