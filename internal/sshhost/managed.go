...
// OperationLockPath derives the one HOME-scoped lock file from the canonical
// ~/.ssh/dev.d resource without consulting dev configuration or creating an SSH
// directory.
func OperationLockPath(paths Paths) (string, error) {
	if err := paths.Validate(); err != nil {
		return "", err
	}
	return filepath.Join(paths.Home, ".dev-ssh-"+filepath.Base(paths.ManagedDir)+".operations.lock"), nil
}

// WithOperationLock runs operation while holding the deterministic explicit
// lock derived from ~/.ssh/dev.d. The existing HOME is validated, and lock
// acquisition creates no directory or managed-namespace entry.
func WithOperationLock(ctx context.Context, paths Paths, operation func() error) error {
	if operation == nil {
		return errors.New("SSH operation lock requires an operation")
	}
	if ctx == nil {
	...
	lockPath, err := OperationLockPath(paths)
	if err != nil {
		return err
	}
	return lockx.WithFile(ctx, lockPath, "SSH operation", operation)
}
...
func ValidateManagedDefinition(definition ManagedDefinition) error {
	...
	for name, value := range map[string]string{
		...
		if value != "" && !validUTF8NoControl(value) {
...
func RenderManaged(definition ManagedDefinition) ([]byte, error) {
	...
	body.WriteByte('\n')
	writeConfigDirective(&body, "HostName", definition.HostName)
	writeConfigDirective(&body, "User", definition.User)
	if definition.Port != 0 {
		writeConfigDirective(&body, "Port", strconv.Itoa(definition.Port))
	}
	writeConfigDirective(&body, "ProxyJump", definition.ProxyJump)
	writeConfigDirective(&body, "IdentityFile", definition.IdentityFile)
	if definition.IdentitiesOnly != nil {
		value := "no"
...
func writeConfigDirective(body *strings.Builder, name, value string) {
	if value == "" {
		return
	}
	body.WriteString("    ")
...
func (s *Service) ApplyManaged(ctx context.Context, plan ManagedPlan) (ManagedResult, error) {
	...
	err := lockx.WithDir(ctx, s.paths.ManagedDir, "SSH host config", func() error {
		...
		if s.afterManagedCommit != nil {
			s.afterManagedCommit()
		}
		...
		verifyOrRollback := func(verifyErr error) error {
			rollbackErr := s.rollbackManagedPublication(plan, written)
			if rollbackErr == nil {
				result.Changed = false
				result.RolledBack = true
				result.Digest = plan.BeforeDigest
				return fmt.Errorf("verify managed SSH config (publication rolled back): %w", verifyErr)
			}
			return errors.Join(fmt.Errorf("verify managed SSH config: %w", verifyErr), fmt.Errorf("rollback managed publication: %w", rollbackErr))
		}
		if err := s.revalidateManagedInventory(ctx, plan.state.alias, plan.state.path); err != nil {
			return verifyOrRollback(err)
		}
...
func (s *Service) revalidateManagedInventory(ctx context.Context, alias, path string) error {
	...
	if !inventory.Complete {
		return fmt.Errorf("SSH config closure became incomplete: %w", ErrSourceChanged)
	}
	if !inventory.ManagedIncludeActive {
...
func (s *Service) verifyManagedSnapshot(ctx context.Context, definition ManagedDefinition, expected fileSnapshot) error {
	...
	// ssh -G is effectful: Match exec and other configured helpers may mutate the
	// active closure. Re-prove completeness and collision freedom only after that
	// subprocess returns, then confirm the published identity is still ours.
	if err := s.revalidateManagedInventory(ctx, definition.Alias, expected.path); err != nil {
		return fmt.Errorf("revalidate managed SSH config after ssh -G: %w", err)
	}
...
func (s *Service) managedCollisionDiagnostics(alias, target string, inventory Inventory) []Diagnostic {
	...
	for _, diagnostic := range inventory.Diagnostics {
		if diagnostic.BlocksMutation {
			...
			incompleteBlocked = incompleteBlocked || diagnostic.Incomplete
		}
	}
	if !inventory.Complete && !incompleteBlocked {
		diagnostics = append(diagnostics, Diagnostic{
			Code: "discovery_incomplete", Message: "the active SSH config closure could not be proven complete",
			Path: inventory.Root, Incomplete: true, BlocksMutation: true,
		})
	}
