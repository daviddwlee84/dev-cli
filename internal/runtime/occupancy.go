...
func InspectOccupancy(ctx context.Context, rt Runtime, target string, opts OccupancyOptions) (Occupancy, error) {
	...
	if listErr != nil {
		...
		for _, observed := range sessions {
			covering, mixed, classifyErr := classifyOccupancySession(checkout.path, observed)
			if classifyErr != nil {
...
func appendOccupancyAgent(ctx context.Context, result *Occupancy, checkout checkoutIdentity, opts OccupancyOptions, activity AgentActivity) error {
	covered, err := activityCoversCheckout(ctx, checkout, activity.CWD)
	if err != nil {
...
func inspectCheckoutIdentity(ctx context.Context, dir string) (checkoutIdentity, error) {
	...
	if discoverErr == nil && repo.Root != "" {
		...
		return checkoutIdentity{path: root, git: true}, nil
	}
...
func activityCoversCheckout(ctx context.Context, target checkoutIdentity, cwd string) (bool, error) {
	if cwd == "" {
	...
	if target.git && activity.git {
		return target.path == activity.path, nil
	}
	return pathx.Contains(target.path, activity.path)
}

func classifyOccupancySession(target string, session Session) ([]Pane, []Pane, error) {
	panes := session.Panes
	if len(panes) == 0 {
