...
func helpGuideEntries(view View) []helpEntry {
	...
	case ViewRemote:
		...
		add("pull-requests", "Using this view", "Space expands pull requests; Ctrl+O selects a PR action", "GitHub and GitLab repository children load only on explicit expansion. The default page is 50; repositories with more than 50 open requests initially show authored requests and requested reviews, with the scope and overall count visible. Actions can show all, load more or refresh. Session data becomes stale after five minutes; ordinary r, cursor movement and / never fetch request details. / searches loaded rows only. Enter opens or prepares an exact PR checkout; diff, provisioning and merge are separate actions.", "pull-requests")
		add("pr-colors", "Colors and symbols", "PR labels retain meaning without color", "Mine and review-requested badges identify your relationship. Failing checks are red; pending checks, conflicts and requested changes are amber; drafts are dim. Green ready requires an explicit fresh detail observation and remains advisory. Passing checks alone never mean ready to merge. The action service revalidates before any mutation.", "pull-requests")
