...
func TestGitignoreAgentArtifactContract(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "SpecStory statistics", path: ".specstory/statistics.json", wantIgnored: true},
		{name: "SpecStory history", path: ".specstory/history/session.md"},
		{name: "SpecStory project identity", path: ".specstory/.project.json"},
		{name: "SpecStory config", path: ".specstory/cli/config.toml"},
