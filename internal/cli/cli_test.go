...
func TestPrepareSpecStoryPathSelectsExactDuplicate(t *testing.T) {
	h := newHarness(t)
	const session = "codex:01a0438b-5d41-7e60-b11f-ef9f2ab4c7b2"
	data := "<!-- Generated by SpecStory, Markdown v2.1.0 -->\n# Review\n<!-- Codex CLI Session 01a0438b-5d41-7e60-b11f-ef9f2ab4c7b2 (now) -->\nbody\n"
	h.repo.Write(".specstory/history/selected.md", data)
	h.repo.Write(".specstory/history/alias.md", data)
	if _, _, err := h.run("prepare", h.repo.Root, "--session", session); err == nil || !strings.Contains(err.Error(), "multiple") {
		t.Fatalf("duplicate lookup=%v", err)
	}
	out := h.mustRun("prepare", h.repo.Root, "--session", session, "--specstory-path", ".specstory/history/selected.md", "--run-id", "exact-path-cli")
	if !strings.Contains(out, "PREPARED") {
		t.Fatalf("output=%q", out)
	}
	store := artifact.NewStore(filepath.Join(h.home, "state", "artifact-intents", "v1"))
	intent, err := store.FindByRunID("exact-path-cli")
	if err != nil {
		t.Fatal(err)
	}
	path, _ := filepath.EvalSymlinks(filepath.Join(h.repo.Root, ".specstory/history/selected.md"))
	if intent.SpecStoryPath != path {
		t.Fatalf("stored selector=%q want %q", intent.SpecStoryPath, path)
	}
	if _, _, err := h.run("prepare", h.repo.Root, "--session", "claude:01a0438b-5d41-7e60-b11f-ef9f2ab4c7b2", "--specstory-path", path); err == nil {
		t.Fatal("CLI allowed wrong provider")
	}
}

type artifactReviewWriter struct {
	onReview func()
	bytes.Buffer
}

func (w *artifactReviewWriter) Write(p []byte) (int, error) {
	if w.onReview != nil && bytes.Contains(p, []byte("DISCARDING")) {
		f := w.onReview
		w.onReview = nil
		f()
	}
...
func TestArtifactDiscardRefusesRevisionChangedAfterReview(t *testing.T) {
	h := newHarness(t)
	dir := filepath.Join(h.home, "state", "artifact-intents", "v1")
	if err := os.MkdirAll(dir, 0o700); err != nil {
		t.Fatal(err)
	}
	writeIntentJSON(t, dir, "intent-reviewed", h, "failed")
	store := artifact.NewStore(dir)
	out := &artifactReviewWriter{onReview: func() {
		if err := store.Update(t.Context(), "intent-reviewed", func(i *artifact.Intent) error {
			i.Status = artifact.Finalized
			i.ArtifactCommit = "new-receipt"
			return nil
		}); err != nil {
			t.Fatal(err)
		}
	...
	root := cli.NewRootCommandWithIO(out, &errOut)
	root.SetArgs([]string{"--config", h.configPath, "artifact", "discard", "intent-reviewed", "--yes"})
	if err := root.Execute(); !errors.Is(err, artifact.ErrStaleRevision) {
		t.Fatalf("stale discard=%v", err)
	}
	intent, err := store.Get("intent-reviewed")
	if err != nil || intent.Status != artifact.Finalized || intent.ArtifactCommit != "new-receipt" {
