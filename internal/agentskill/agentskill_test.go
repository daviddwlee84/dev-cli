...
func TestScanImmediateChildrenWithBoundedFrontmatter(t *testing.T) {
	...
	if len(result.Diagnostics) != 2 {
		t.Fatalf("diagnostics = %+v, want malformed and oversized", result.Diagnostics)
	}
...
func TestNormalizedPhysicalCollisionMergesButFindProjectFailsClosed(t *testing.T) {
	isolateAgentEnvironment(t)
	repository := initRepository(t)
	writeSkill(t, filepath.Join(repository, ".agents", "skills", "one"), "Foo Bar", "")
	writeSkill(t, filepath.Join(repository, ".claude", "skills", "two"), "foo-bar", "")

	result, err := Inventory(context.Background(), repository, ListOptions{Project: true})
	if err != nil {
		t.Fatal(err)
	}
	if len(result.Skills) != 1 || len(result.Skills[0].Names) != 2 || len(result.Skills[0].Installations) != 2 {
		t.Fatalf("normalized collision row = %+v", result.Skills)
	}
	if len(result.Diagnostics) != 1 || result.Diagnostics[0].Kind != DiagnosticNameCollision {
		t.Fatalf("collision diagnostics = %+v", result.Diagnostics)
	}
	for _, name := range []string{"Foo Bar", "foo-bar", "foo_bar"} {
		if _, err := FindProject(context.Background(), repository, name); err == nil || !strings.Contains(err.Error(), "ambiguous") {
			t.Errorf("FindProject(%q) error = %v, want ambiguity", name, err)
		}
	}
}
...
func TestPresenceExternalAndBundledIntegrityAreIndependentFromFreshness(t *testing.T) {
	...
	if byName["missing"].Presence != PresenceMissing || byName["missing"].ManagedBy != ManagedBySkills {
	...
	if dev.ManagedBy != ManagedByDev || dev.Integrity != IntegrityVerified || dev.UpdateStatus != UpdateUnknown {
	...
	if got := skillsByName(result.Skills)[bundledskill.Name]; got.Integrity != IntegrityDrifted || got.UpdateStatus != UpdateUnknown {
