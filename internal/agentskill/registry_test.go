...
func TestRegistrySharedDirectoriesAreScannedOnceWithAllAttribution(t *testing.T) {
	isolateAgentEnvironment(t)
	root := t.TempDir()
	locations := projectScanLocations(root, Registry())
	sharedPath := filepath.Join(root, ".agents", "skills")
	matches := 0
	for _, location := range locations {
		for _, logical := range location.Logical {
			if logical.Path != sharedPath {
				continue
			}
			matches++
			if len(logical.AgentIDs) < 10 || !contains(logical.AgentIDs, "universal") || !contains(logical.AgentIDs, "codex") {
				t.Fatalf("shared path attribution = %v", logical.AgentIDs)
			}
		}
	}
	if matches != 1 {
		t.Fatalf("shared registry path appears %d times, want once", matches)
	}
}
