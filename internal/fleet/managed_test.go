...
`)
	...
	if cfg.ManagedFragmentDirectory() != filepath.Join(root, "lab.d") {
		t.Fatalf("managed directory = %q", cfg.ManagedFragmentDirectory())
	}
...
func TestManagedFragmentWriterAndRemoverSeamsAreIdempotent(t *testing.T) {
	...
	writer := ManagedFragmentWriteFunc(func(request ManagedFragmentWriteRequest) error {
		writes++
		if request.Existed || request.DirectoryMode != 0o700 || request.FileMode != 0o600 {
			t.Fatalf("unexpected write request: %+v", request)
		}
		if err := os.MkdirAll(filepath.Dir(request.Path), request.DirectoryMode); err != nil {
			return err
		}
		if err := os.Chmod(filepath.Dir(request.Path), request.DirectoryMode); err != nil {
			return err
		}
		return os.WriteFile(request.Path, request.Content, request.FileMode)
	})
	path, err := WriteManagedFragment(primary, host, writer)
	if err != nil {
		t.Fatal(err)
	}
	if writes != 1 {
	...
	if _, err := WriteManagedFragment(primary, host, writer); err != nil {
		t.Fatal(err)
	}
	if writes != 1 {
	...
	removedPath, err := RemoveManagedFragment(primary, host.SSHAlias, remover)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := RemoveManagedFragment(primary, host.SSHAlias, remover); err != nil {
		t.Fatal(err)
	}
	if removes != 1 {
...
func TestRemoveManagedFragmentRefusesManualDrift(t *testing.T) {
	...
	called := false
	_, err = RemoveManagedFragment(primary, host.SSHAlias, ManagedFragmentRemoveFunc(func(ManagedFragmentRemoveRequest) error {
		called = true
		return nil
	}))
	if err == nil || called {
...
func TestWriteManagedFragmentRefusesInvalidMergedConfig(t *testing.T) {
	...
	called := false
	path, err := WriteManagedFragment(primary, ManagedHost{
