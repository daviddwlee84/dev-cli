...
func TestFleetBackgroundRefreshOverlay(t *testing.T) {
	if !Default().TUI.Fleet.BackgroundRefresh {
		t.Fatal("fleet background refresh must default to enabled")
	}
	path := filepath.Join(t.TempDir(), "config.toml")
	if err := os.WriteFile(path, []byte("[tui.fleet]\nbackground_refresh = false\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	cfg, err := Load(path)
	if err != nil {
	...
	if cfg.TUI.Fleet.BackgroundRefresh {
		t.Fatal("explicit false must disable fleet background refresh")
	}
	if cfg.EffectiveRepoSort() != "activity" {
		t.Fatal("fleet setting changed unrelated repository defaults")
	}
}
