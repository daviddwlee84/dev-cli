...
// ReadTranscriptSession proves one regular file's strict bounded SpecStory preamble.
func ReadTranscriptSession(path string) (provider, sessionID string, err error) {
	return agenthistory.ReadTranscriptSession(path)
}

// SelectTranscript binds a strict session proof to an exact checkout history path.
func SelectTranscript(worktree, path, provider, sessionID string) (Transcript, error) {
	selected, err := agenthistory.SelectTranscript(worktree, path, provider, sessionID)
	if err != nil {
		return Transcript{}, err
	}
	root, err := pathx.Canonical(worktree)
	if err != nil {
		return Transcript{}, err
	}
	rel, err := filepath.Rel(root, selected)
	if err != nil || !strings.HasPrefix(filepath.ToSlash(rel), ".specstory/history/") {
		return Transcript{}, fmt.Errorf("selected transcript is outside .specstory/history")
	}
	return Transcript{Path: selected, Provider: provider, SessionID: sessionID}, nil
}
