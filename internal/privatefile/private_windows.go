...
func Descriptor(dir bool) (*windows.SECURITY_DESCRIPTOR, error) {
...
func MakeDir(path string) error {
	descriptor, err := Descriptor(true)
	if err != nil {
		return err
	}
...
func Check(path string, _ fs.FileInfo, dir bool) error {
	pointer, err := windows.UTF16PtrFromString(path)
	if err != nil {
		return err
	}
	...
	if err != nil {
		return errors.New("private artifact identity unavailable")
	}
	...
	if windows.GetFileInformationByHandle(handle, &info) != nil || info.FileAttributes&windows.FILE_ATTRIBUTE_REPARSE_POINT != 0 || !dir && info.NumberOfLinks != 1 {
		return errors.New("unsafe private artifact identity")
	}
	...
	if err != nil || !owner.Equals(current.User.Sid) {
		return errors.New("private artifact owner differs from current user")
	}
	control, _, err := descriptor.Control()
	if err != nil || control&windows.SE_DACL_PROTECTED == 0 {
		return errors.New("private artifacts require a protected DACL")
	}
	acl, _, err := descriptor.DACL()
	if err != nil || acl == nil || acl.AceCount != 2 {
		return errors.New("private artifacts require current-user and SYSTEM access only")
	}
