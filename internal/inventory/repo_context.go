...
func CollectRepoContextWithOptions(ctx context.Context, r repo.Repo, tasks []*task.Task,
	...
	for _, worktree := range worktrees {
		...
		checkout := RepoCheckout{
			...
			Worktree:    worktree, Exists: pathIsDir(worktree.Path),
	...
	enrichRepoCheckouts(ctx, out.Checkouts, opts.Limiter)
...
func enrichRepoCheckouts(ctx context.Context, checkouts []RepoCheckout, limiter *Limiter) {
	for i := range checkouts {
		checkout := &checkouts[i]
		if checkout.Worktree.Bare || checkout.Worktree.Prunable || !checkout.Exists {
			continue
		}
		release := func() {}
		if limiter != nil {
...
func taskExpectsCheckout(tracked *task.Task) bool {
	return tracked != nil && tracked.State != task.Cold && tracked.State != task.Done
}

func checkoutActionable(checkout RepoCheckout) bool {
	return !checkout.Worktree.Bare && !checkout.Worktree.Prunable && checkout.Exists && checkout.StatusErr == nil
}
