...
func TestWithLockSerializesCompatibilityWriters(t *testing.T) {
	...
	if err := <-compatibilityDone; !errors.Is(err, task.ErrConflict) {
		t.Fatalf("compatibility Save after transaction = %v, want stale-write conflict", err)
	}
	final, err := s.Get(created.Task.ID)
	if err != nil {
		t.Fatal(err)
	}
	if final.Next != "transaction" {
		t.Fatalf("stale compatibility writer replaced transaction: %+v", final)
	}
}
